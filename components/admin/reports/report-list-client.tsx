...
export function ReportListClient({ initialReports }: ReportListClientProps) {
  ...
  const getTargetLink = (targetId: string, targetType: Report["targetType"]) => {
    switch (targetType) {
      case "DESTINATION":
      ...
      <Table>
        ...
        <TableBody>
          {reports.length === 0 ? (
            ...
            reports.map((report) => (
              <TableRow key={report.id} className="hover:bg-stone-50/50 transition-colors">
                ...
                <TableCell>
                  {report.reporter ? (
                    ...
                    <span className="text-muted-foreground italic text-sm">Anonim</span>
                  )}
                </TableCell>
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Tinjau detail laporan dan perbarui status penanganan.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-6 py-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  <Label className="text-muted-foreground">Tipe Target</Label>
                  <p className="font-medium uppercase tracking-tight">{selectedReport.targetType}</p>
                </div>
                <div className="space-y-1">
                  <Label className="text-muted-foreground">ID Target</Label>
                  <p className="font-mono text-xs">{selectedReport.targetId}</p>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  <Label className="text-muted-foreground">Waktu Laporan</Label>
                  <p className="font-medium">
                    {format(new Date(selectedReport.createdAt), "dd MMMM yyyy, HH:mm", { locale: localeId })}
                  </p>
                </div>
              </div>

              <div className="space-y-2">
                <Label className="text-muted-foreground">Deskripsi Pelapor</Label>
