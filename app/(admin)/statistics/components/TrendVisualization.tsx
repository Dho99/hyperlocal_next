...
export default function TrendVisualization({ trend }: TrendVisualizationProps) {
    ...
    const maxValue = Math.max(
        1,
        ...trend.map((t) => Math.max(t.submissions, t.approvals)),
    );

    // Convert trend data to SVG points
    const submissionPoints = trend
        .map((t, i) => {
        ...
        <div className="bg-white dark:bg-card rounded-xl p-6 border border-gray-200 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <h2 className="text-lg font-bold text-emerald-900 dark:text-card-foreground">
                    Validation Activity Trend (Last 4 Weeks)
                </h2>

                <div className="flex items-center gap-6 bg-slate-50 dark:bg-card-foreground/10 px-4 py-2 rounded-lg border border-gray-100">
                    <div className="flex items-center gap-2.5">
                        <div className="w-5 border-t-2 border-dashed border-gray-400"></div>
                        <span className="text-sm font-bold text-gray-500 dark:text-card-foreground">
                            Submissions
                        </span>
                    </div>
                    <div className="flex items-center gap-2.5">
                        <div className="w-5 border-t-2 border-solid border-emerald-600"></div>
                        <span className="text-sm font-bold text-gray-700 dark:text-card-foreground">
                            Approvals
                        </span>
                    </div>
                </div>
            ...
            <div className="relative w-full h-[320px] pt-4 overflow-hidden">
                ...
                >
                    ...
                    {[0, 1, 2, 3, 4].map((i) => (
                        <g key={i}>
                            ...
                            <text
                                ...
                                y={i * 55 + 24}
                                textAnchor="end"
                                className="text-xs fill-gray-400 dark:text-card-foreground font-bold"
                            >
                    ...
                    {trend.map((t, i) => (
                        <text
                            key={i}
                            x={40 + (i * 960) / (trend.length - 1)}
                            y="270"
                            textAnchor="middle"
                            className="text-xs fill-gray-400 dark:text-card-foreground font-bold uppercase tracking-wider"
                        >
                            {t.label}
                        </text>
                    ))}
