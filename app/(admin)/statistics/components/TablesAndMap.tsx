    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
      ...
      <div className="xl:col-span-2 bg-white rounded-xl p-6 border border-gray-200 shadow-sm overflow-hidden flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <h2 className="text-lg font-bold text-emerald-900">Top Rankings Overview</h2>
                  ...
                  <td className="py-4.5 pr-6">
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-emerald-600 rounded-full" style={{ width: `${Math.min(100, (item.totalScore / 3) * 100)}%` }}></div>
                      </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm flex flex-col">
        <h2 className="text-lg font-bold text-emerald-900 mb-6">GIS & Heatmap Analytics</h2>
        <div className="relative flex-1 w-full min-h-[320px] bg-slate-50 rounded-xl overflow-hidden border border-gray-200 shadow-inner">
          ...
          {mapData.slice(0, 12).map((d, i) => {
            ...
            return (
              <div key={d.id} className="absolute transform -translate-x-1/2 -translate-y-1/2 group" style={{ top: `${top}%`, left: `${left}%` }}>
                <MapPin className={`w-5 h-5 drop-shadow-sm z-10 hover:scale-125 transition-transform ${d.intensity > 0.8 ? 'text-emerald-700' : d.intensity > 0.5 ? 'text-emerald-500' : 'text-emerald-300'}`} fill="currentColor" />
          ...
          <div className="absolute bottom-5 left-5 right-5">
            <div className="bg-white/95 backdrop-blur-md border border-emerald-100 shadow-lg px-4 py-3 rounded-xl">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-emerald-900">Heatmap Intensity</span>
